...
# from bert_seq2seq.paddle_model.data import vocab
...
class BasicTokenizer(object):
    ...
    def __init__(self):
        """初始化
        """
        # self._token_to_id = word2idx if word2idx is not None else {}
        # self._token_unk_id = self._token_to_id.get("[UNK]", 100)
        self._token_pad = '[PAD]'
        self._token_cls = '[CLS]'
        self._token_sep = '[SEP]'
        self._token_unk = '[UNK]'
    ...
    # 一组 token 转 id 序列（List[token] → List[int]）
    ...
    #     return self._token_dict_inv.get(idx, "[UNK]")
    def id_to_token(self, i):
        """id序列为对应的token
        """
        raise NotImplementedError
...
class Tokenizer(BasicTokenizer):
    ...
    def __init__(self, token_dict):
        ...
        # self._token_unk_id = vocab.get('[UNK]', 100)
        self._token_dict = token_dict
        self._token_dict_inv = {v: k for k, v in token_dict.items()}
        
        required_tokens = ['pad', 'cls', 'sep', 'unk', 'mask']
        for token in required_tokens:
            token_str = getattr(self, f"_token_{token}")
            if token_str not in token_dict:
                raise ValueError(f"[ERROR] Special token '{token_str}' not found in vocab.")
            token_id = token_dict[token_str]
            setattr(self, f"_token_{token}_id", token_id)
...
if __name__ == "__main__":
    ...
    encode_text, _ = tokenizer.encode(text)
    encode_text = encode_text[1:-1]

    print(f"encode text is {encode_text}")
    # # print(segment_ids)
