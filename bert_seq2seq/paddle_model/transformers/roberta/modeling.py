...
class RobertaForQuestionAnswering(RobertaPretrainedModel):
    ...
    def __init__(self, roberta, dropout=None):
        ...
        self.roberta = roberta  # allow roberta to be model
...
class robertaLMPredictionHead(nn.Layer):
    """
...
class robertaSeq2Seq(RobertaPretrainedModel):
    ...
    def __init__(self, roberta, ):
        ...
        self.predictions = robertaLMPredictionHead(
            ...
            vocab_size=roberta.config["vocab_size"],
            activation=roberta.config["hidden_act"],
            embedding_weights=roberta.embeddings.word_embeddings.weight)

    def _make_seq_mask(self, token_type_ids):
        if token_type_ids is None:
            return None

        seq_len = token_type_ids.shape[1]
        ## 构建特殊的mask
        ones = paddle.ones((1, 1, seq_len, seq_len), dtype="float32")
        a_mask = paddle.tril(ones)
        s_ex12 = token_type_ids.unsqueeze(1).unsqueeze(2).astype("float32")
        s_ex13 = token_type_ids.unsqueeze(1).unsqueeze(3).astype("float32")
        a_mask = (1.0 - s_ex12) * (1.0 - s_ex13) + s_ex13 * a_mask
        # print(f"a_mask is {a_mask}")
        return a_mask

    def _compute_loss(self, predictions, labels, target_mask):
        """
        target_mask : 句子a部分和pad部分全为0， 而句子b部分为1
        """
        predictions = predictions.reshape((-1, self.roberta.config["vocab_size"]))
        labels = labels.reshape([-1, ])
        target_mask = target_mask.reshape([-1, ]).astype("float32")
        # print(f"labels is {labels}")
        # print(f"target mask is {target_mask}")
        # print(f"predictions is {predictions}")
        loss = nn.loss.CrossEntropyLoss(ignore_index=0, reduction="none")
    ...
    def forward(self,
        ...
        extended_attention_mask = (input_ids > 0).astype("float32")  ## batch seq_len
        # 注意力矩阵mask: [batch_size, 1, 1, seq_length]
        extended_attention_mask = extended_attention_mask.unsqueeze(1).unsqueeze(2)

        attention_mask = self._make_seq_mask(token_type_ids)
        ...
        if label is not None:
            ...
            loss = self._compute_loss(predictions, label, target_mask)
            return loss
        else:
            return predictions
