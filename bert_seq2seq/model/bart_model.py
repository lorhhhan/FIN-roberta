...
def shift_tokens_right(input_ids: torch.Tensor, pad_token_id: int, decoder_start_token_id: int):
    ...
    assert pad_token_id is not None, "self.model.model.pad_token_id has to be defined."
    ...
    def forward(
        ...
        Args:
            ...
            encoder_layer_head_mask (:obj:`torch.FloatTensor`): mask for encoder attention heads in a given layer of
                size `(model.encoder_attention_heads,)`.
...
class BartEncoder(nn.Module):
    """
    Transformer encoder consisting of *model.encoder_layers* self attention layers. Each layer is a
...
class BartDecoder(nn.Module):
    """
    Transformer decoder consisting of *model.decoder_layers* layers. Each layer is a :class:`BartDecoderLayer`
    ...
    def forward(
        ...
        Args:
            ...
            past_key_values (:obj:`Tuple[Tuple[torch.Tensor]]` of length :obj:`model.n_layers` with each tuple having 2 tuples each of which has 2 tensors of shape :obj:`(batch_size, num_heads, sequence_length - 1, embed_size_per_head)`):
...
class BartForConditionalGeneration(nn.Module):
    ...
    def forward(
        ...
        labels (:obj:`torch.LongTensor` of shape :obj:`(batch_size, sequence_length)`, `optional`):
            ...
            model.vocab_size]`` or -100 (see ``input_ids`` docstring). Tokens with indices set to ``-100`` are ignored
            (masked), the loss is only computed for the tokens with labels in ``[0, ..., model.vocab_size]``.
...
if __name__ == "__main__":
    ...
    # model2 = BartForConditionalGeneration(model)
